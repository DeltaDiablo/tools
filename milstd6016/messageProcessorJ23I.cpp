#include "messageProcessorJ23I.h"
...
WordMessage ParseJ23IWordBitsInternal(const std::string& msbToLsbBits)
{
	static const std::vector<internal::FieldSpec> fields = {
		{"WORD FORMAT", 1550, 1, 0, 1, "Expected fixed coding 00 for J2.3I."},
		{"LABEL, J-SERIES", 270, 4, 2, 6, "Expected fixed coding 00010 for J2.3I."},
		{"SUBLABEL, J-SERIES", 271, 5, 7, 9, "Expected fixed coding 011 for J2.3I."},
		...
		{"SPARE (SP)", 756, 1, 14, 14, "Spare bit."},
		...
		{"ELEVATION, 25 FT", 1612, 1, 40, 50, "Elevation of the JTIDS antenna."},
	...
	return internal::BuildWordMessage(msbToLsbBits, "J2.3I", "SURFACE PPLI INITIAL WORD", fields);
}
...
J23IMessage ParseJ23IMessageBits(const std::string& wordBits)
{
	...
	{
		J23IMessage message;
		message.error = "Invalid J2.3I word length: expected 70 bits, received " + std::to_string(bits.size()) + ".";
		return message;
	}

	return ParseJ23IWordBitsInternal(bits);
}
...
J23IMessage ParseJ23IMessageBytes(const std::vector<int>& bytes)
{
	...
	{
		J23IMessage message;
		message.error = "Invalid J2.3I input: no bytes were provided.";
		return message;
	}
	...
	{
		J23IMessage message;
		message.error = "Invalid J2.3I byte input: expected at least 70 bits, received " + std::to_string(bits.size()) + ".";
		return message;
	}

	return ParseJ23IWordBitsInternal(bits.substr(0, 70));
}
...
J23IMessage ParseJ23IMessage(const std::string& input)
{
	...
	{
		return ParseJ23IMessageBits(input);
	}

	std::vector<int> bytes;
	if (internal::TryParseCsvBytes(input, bytes))
	{
		return ParseJ23IMessageBytes(bytes);
	}

	J23IMessage message;
	message.error = "Invalid J2.3I input: expected a 70-bit binary string or comma-separated byte values.";
	return message;
}

std::string ProcessJ23IMessageBits(const std::string& wordBits)
{
	return internal::FormatWordMessage(ParseJ23IMessageBits(wordBits));
}

std::string ProcessJ23IMessageBytes(const std::vector<int>& bytes)
{
	return internal::FormatWordMessage(ParseJ23IMessageBytes(bytes));
}

std::string ProcessJ23IMessage(const std::string& input)
{
	return internal::FormatWordMessage(ParseJ23IMessage(input));
}
