#include "messageProcessorJ26I.h"
...
WordMessage ParseJ26IWordBitsInternal(const std::string& msbToLsbBits)
{
	static const std::vector<internal::FieldSpec> fields = {
		{"WORD FORMAT", 1550, 1, 0, 1, "Expected fixed coding 00 for J2.6I."},
		{"LABEL, J-SERIES", 270, 4, 2, 6, "Expected fixed coding 00010 for J2.6I."},
		{"SUBLABEL, J-SERIES", 271, 5, 7, 9, "Expected fixed coding 110 for J2.6I."},
	...
	return internal::BuildWordMessage(msbToLsbBits, "J2.6I", "LAND TRACK PPLI INITIAL WORD", fields);
}
...
J26IMessage ParseJ26IMessageBits(const std::string& wordBits)
{
	...
	{
		J26IMessage message;
		message.error = "Invalid J2.6I word length: expected 70 bits, received " + std::to_string(bits.size()) + ".";
		return message;
	}

	return ParseJ26IWordBitsInternal(bits);
}
...
J26IMessage ParseJ26IMessageBytes(const std::vector<int>& bytes)
{
	...
	{
		J26IMessage message;
		message.error = "Invalid J2.6I input: no bytes were provided.";
		return message;
	}
	...
	{
		J26IMessage message;
		message.error = "Invalid J2.6I byte input: expected at least 70 bits, received " + std::to_string(bits.size()) + ".";
		return message;
	}

	return ParseJ26IWordBitsInternal(bits.substr(0, 70));
}
...
J26IMessage ParseJ26IMessage(const std::string& input)
{
	...
	{
		return ParseJ26IMessageBits(input);
	}

	std::vector<int> bytes;
	if (internal::TryParseCsvBytes(input, bytes))
	{
		return ParseJ26IMessageBytes(bytes);
	}

	J26IMessage message;
	message.error = "Invalid J2.6I input: expected a 70-bit binary string or comma-separated byte values.";
	return message;
}

std::string ProcessJ26IMessageBits(const std::string& wordBits)
{
	return internal::FormatWordMessage(ParseJ26IMessageBits(wordBits));
}

std::string ProcessJ26IMessageBytes(const std::vector<int>& bytes)
{
	return internal::FormatWordMessage(ParseJ26IMessageBytes(bytes));
}

std::string ProcessJ26IMessage(const std::string& input)
{
	return internal::FormatWordMessage(ParseJ26IMessage(input));
}
